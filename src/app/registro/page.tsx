...
export default function RegistroPage() {
  ...
  async function handleRegistro(e: React.FormEvent) {
    ...
    if (error) { setError('Error al crear la cuenta. Verificá los datos.'); setLoading(false); return }
    if (data.user) {
        ...
        <div className="relative z-10 w-full max-w-sm flex flex-col items-center gap-6 text-center">
          ...
          <div className="w-full rounded-2xl px-6 py-8 flex flex-col items-center gap-4" style={{ background: 'rgba(11,31,58,0.75)', backdropFilter: 'blur(14px)', border: '1px solid rgba(45,212,191,0.3)' }}>
            ...
            <p style={{ color: 'rgba(247,250,255,0.6)', fontSize: '14px', lineHeight: '1.5' }}>
              Te enviamos un email de confirmación a <strong style={{ color: '#F7FAFF' }}>{email}</strong>. Revisá tu bandeja de entrada.
            </p>
            <a href="/login" className="w-full rounded-xl font-semibold flex items-center justify-center"
              style={{ height: '48px', background: '#1E6AC8', color: '#F7FAFF', fontSize: '15px' }}>
              Ir al login
            </a>
          </div>
        </div>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-10 relative overflow-hidden" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="relative z-10 w-full max-w-sm flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4 rounded-2xl px-6 py-7" style={{ background: 'rgba(11,31,58,0.75)', backdropFilter: 'blur(14px)', border: '1px solid rgba(74,123,181,0.25)' }}>
          ...
          <form onSubmit={handleRegistro} className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              ...
              <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Juan García" required
                className="w-full rounded-xl px-4 outline-none transition-all" style={inputStyle}
                onFocus={e => (e.target.style.borderColor = 'rgba(30,106,200,0.6)')} onBlur={e => (e.target.style.borderColor = 'rgba(74,123,181,0.3)')} />
            </div>
            <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                  ...
                  className="w-full rounded-xl px-4 pr-12 outline-none transition-all" style={inputStyle}
                  onFocus={e => (e.target.style.borderColor = 'rgba(30,106,200,0.6)')} onBlur={e => (e.target.style.borderColor = 'rgba(74,123,181,0.3)')} />
            ...
            <button type="submit" disabled={loading} className="w-full rounded-xl font-semibold flex items-center justify-center gap-2 transition-opacity"
          ...
          <button onClick={handleGoogle} disabled={loadingGoogle} className="w-full rounded-xl flex items-center justify-center gap-3 transition-opacity"
