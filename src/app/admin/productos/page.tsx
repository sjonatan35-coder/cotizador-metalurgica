...
const badgeStyle = (activo: boolean): React.CSSProperties => ({
  ...
  background: activo ? '#EAF3DE' : '#f1efe8', color: activo ? '#3B6D11' : '#5f5e5a',
...
export default function AdminProductos() {
  ...
  const [form, setForm] = useState({
    nombre: '', tipo: 'Chapa', subtipo: '', unidades: [] as string[],
    descripcion: '', activo: true, tipoNuevo: '',
  })
  ...
  const guardarProducto = async () => {
    ...
    if (!tipoFinal) { mostrarMensaje('Escribí el nombre del tipo nuevo', 'error'); return }
    if (form.unidades.length === 0) { mostrarMensaje('Seleccioná al menos una unidad de venta', 'error'); return }
    setCargando(true)
    const payload = {
      nombre: form.nombre.trim(), tipo: tipoFinal, subtipo: form.subtipo || null,
      unidad: form.unidades, descripcion: form.descripcion.trim() || null, activo: form.activo,
    }
    if (editandoId) {
      const { error } = await supabase.from('productos').update(payload).eq('id', editandoId)
      if (error) { mostrarMensaje('Error al actualizar: ' + error.message, 'error') }
      else { mostrarMensaje('Producto actualizado', 'ok'); setEditandoId(null); resetForm(); await cargarProductos() }
    } else {
      const { error } = await supabase.from('productos').insert(payload)
      if (error) { mostrarMensaje('Error al guardar: ' + error.message, 'error') }
      else { mostrarMensaje('Producto guardado', 'ok'); resetForm(); await cargarProductos(); setTab('catalogo') }
    }
    setCargando(false)
  }
  ...
  const toggleActivo = async (producto: Producto) => {
    ...
    if (!error) { mostrarMensaje(producto.activo ? 'Producto desactivado' : 'Producto activado', 'ok'); await cargarProductos() }
  }

  const iniciarEdicion = (p: Producto) => {
    setEditandoId(p.id)
    setForm({ nombre: p.nombre, tipo: p.tipo, subtipo: p.subtipo || '', unidades: Array.isArray(p.unidad) ? p.unidad : [p.unidad], descripcion: p.descripcion || '', activo: p.activo, tipoNuevo: '' })
    setTab('nuevo')
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }

  const procesarCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    setCsvError(null); setCsvPreview([])
    ...
    reader.onload = (ev) => {
      ...
      if (lines.length < 2) { setCsvError('El archivo está vacío o no tiene datos'); return }
      const header = lines[0].toLowerCase().split(',').map(h => h.trim())
      const requiredCols = ['nombre', 'tipo', 'unidad']
      const missing = requiredCols.filter(c => !header.includes(c))
      if (missing.length > 0) { setCsvError(`Faltan columnas: ${missing.join(', ')}`); return }
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        rows.push({
          nombre: row['nombre'], tipo: row['tipo'] || 'Otro', subtipo: row['subtipo'] || null,
          unidad: row['unidad'] ? row['unidad'].split('|').map(u => u.trim()) : ['Unidad'],
          descripcion: row['descripcion'] || null, activo: row['activo'] !== 'false',
        })
      }
      if (rows.length === 0) { setCsvError('No se encontraron filas válidas'); return }
      setCsvPreview(rows)
    }
  ...
  const importarCSV = async () => {
    ...
    if (error) { mostrarMensaje('Error al importar: ' + error.message, 'error') }
    else {
  ...
  const s = {
    ...
    body: { background: '#f7faff', minHeight: 'calc(100vh - 110px)', padding: '16px' } as React.CSSProperties,
    ...
    btnPrimary: { width: '100%', padding: '13px', background: '#1E6AC8', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer', marginTop: 4 } as React.CSSProperties,
  ...
  return (
    <div style={s.page}>
      ...
      {mensaje && (
        <div style={{ position: 'fixed', top: 16, left: '50%', transform: 'translateX(-50%)', zIndex: 9999, background: mensaje.tipo === 'ok' ? '#0F6E56' : '#A32D2D', color: '#fff', padding: '10px 20px', borderRadius: 8, fontSize: 14, fontWeight: 500, boxShadow: '0 4px 16px rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ background: '#0B1F3A', padding: '0 16px 12px', display: 'flex', gap: 8 }}>
        {([
          ...
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ flex: 1, padding: '8px 4px', fontSize: 12, fontWeight: 500, border: 'none', borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, background: tab === t.id ? '#1E6AC8' : 'rgba(247,250,255,0.08)', color: tab === t.id ? '#fff' : 'rgba(247,250,255,0.5)', transition: 'all 0.15s' }}>
            {t.icon} {t.label}
          </button>
        ))}
      </div>
        ...
        {tab === 'nuevo' && (
          <div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input style={s.input} placeholder="Ej: Chapa LAF c16 — 1000x2000" value={form.nombre} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={s.label}>Tipo</label>
              <select style={s.select} value={form.tipo} onChange={e => setForm(f => ({ ...f, tipo: e.target.value, subtipo: '' }))}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {SUBTIPOS[form.tipo].map(sub => (
                    <button key={sub} type="button" onClick={() => setForm(f => ({ ...f, subtipo: f.subtipo === sub ? '' : sub }))}
                      style={{ padding: '7px 16px', fontSize: 13, borderRadius: 99, border: `1px solid ${form.subtipo === sub ? '#1E6AC8' : '#d1dce8'}`, background: form.subtipo === sub ? '#1E6AC8' : '#fff', color: form.subtipo === sub ? '#fff' : '#0B1F3A', cursor: 'pointer', fontWeight: form.subtipo === sub ? 600 : 400, transition: 'all 0.15s' }}>
                      {sub}
                    </button>
                  ))}
                </div>
            ...
            {form.tipo === '__nuevo__' && (
              <div style={{ marginBottom: 14 }}>
                ...
                <input style={s.input} placeholder="Ej: Perfil, Malla, Alambre..." value={form.tipoNuevo} onChange={e => setForm(f => ({ ...f, tipoNuevo: e.target.value }))} />
              </div>
            )}
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 4 }}>
                {UNIDADES.map(u => (
                  <label key={u} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, color: '#0B1F3A', cursor: 'pointer' }}>
                    <input type="checkbox" checked={form.unidades.includes(u)}
                      onChange={e => {
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <textarea style={{ ...s.input, height: 72, resize: 'none', fontFamily: 'inherit' }} placeholder="Calibre, medidas, acabado, notas..." value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))} />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderTop: '1px solid #e2eaf3', marginBottom: 4 }}>
              ...
              <button onClick={() => setForm(f => ({ ...f, activo: !f.activo }))}
                style={{ width: 44, height: 24, borderRadius: 12, border: 'none', cursor: 'pointer', background: form.activo ? '#2DD4BF' : '#d3d1c7', position: 'relative', transition: 'background 0.2s' }}>
                <span style={{ position: 'absolute', top: 3, left: form.activo ? 'calc(100% - 18px)' : 3, width: 18, height: 18, borderRadius: '50%', background: '#fff', transition: 'left 0.2s' }} />
              </button>
            </div>
            <button style={{ ...s.btnPrimary, opacity: cargando ? 0.7 : 1 }} onClick={guardarProducto} disabled={cargando}>
          ...
          <div>
            <div style={{ background: '#fff', border: '1px solid #e2eaf3', borderRadius: 10, padding: 14, marginBottom: 14 }}>
              ...
              <p style={{ fontSize: 11, color: '#4A7BB5', margin: '8px 0 0' }}>
                subtipo: LAF/LAC/GALVANIZADO/ESTAMPADO/CNC — unidad: separá múltiples con | — Ej: Unidad|kg
              </p>
            </div>
            <div style={{ border: '2px dashed #4A7BB5', borderRadius: 10, padding: '24px 16px', textAlign: 'center', marginBottom: 14, cursor: 'pointer', background: '#fff' }} onClick={() => fileRef.current?.click()}>
            ...
            {csvError && (
              <div style={{ background: '#FCEBEB', border: '1px solid #F09595', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#A32D2D', marginBottom: 14 }}>{csvError}</div>
            )}
            {csvPreview.length > 0 && (
              <div>
                <p style={{ fontSize: 13, fontWeight: 600, color: '#0B1F3A', marginBottom: 10 }}>Vista previa — {csvPreview.length} productos</p>
                {csvPreview.slice(0, 5).map((p, i) => (
                  <div key={i} style={s.card}>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      {p.unidad.map(u => <span key={u} style={{ ...tipoBadgeStyle, background: '#FAEEDA', color: '#854F0B' }}>{u}</span>)}
                      <span style={badgeStyle(p.activo)}>{p.activo ? 'Activo' : 'Inactivo'}</span>
                    </div>
                  </div>
                ))}
                {csvPreview.length > 5 && <p style={{ fontSize: 12, color: '#4A7BB5', textAlign: 'center', marginBottom: 10 }}>...y {csvPreview.length - 5} más</p>}
                <button style={{ ...s.btnPrimary, opacity: cargando ? 0.7 : 1 }} onClick={importarCSV} disabled={cargando}>
        ...
        {tab === 'catalogo' && (
          <div>
            <div style={{ position: 'relative', marginBottom: 14 }}>
              ...
              <input style={{ ...s.input, paddingLeft: 36 }} placeholder="Buscar por nombre o tipo..." value={busqueda} onChange={e => setBusqueda(e.target.value)} />
            </div>
            {productosFiltrados.length === 0 && (
              <div style={{ textAlign: 'center', padding: '40px 0', color: '#4A7BB5' }}>
                ...
                {!busqueda && <button onClick={() => setTab('nuevo')} style={{ marginTop: 8, fontSize: 13, color: '#1E6AC8', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Agregar el primero</button>}
              </div>
            )}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: p.descripcion ? 6 : 0 }}>
                      <span style={tipoBadgeStyle}>{p.tipo}</span>
                      {p.subtipo && <span style={{ ...tipoBadgeStyle, background: '#E6F1FB', color: '#185FA5' }}>{p.subtipo}</span>}
                      {Array.isArray(p.unidad) && p.unidad.map(u => <span key={u} style={{ ...tipoBadgeStyle, background: '#FAEEDA', color: '#854F0B' }}>{u}</span>)}
                      <span style={badgeStyle(p.activo)}>{p.activo ? 'Activo' : 'Inactivo'}</span>
                    </div>
                    {p.descripcion && <p style={{ margin: 0, fontSize: 12, color: '#4A7BB5' }}>{p.descripcion}</p>}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
                    <button onClick={() => iniciarEdicion(p)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', fontSize: 12, border: '1px solid #d1dce8', borderRadius: 6, background: '#fff', color: '#1A4B8C', cursor: 'pointer' }}>
                      <Pencil size={12} /> Editar
                    </button>
                    <button onClick={() => toggleActivo(p)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', fontSize: 12, border: `1px solid ${p.activo ? '#F09595' : '#9FE1CB'}`, borderRadius: 6, background: '#fff', color: p.activo ? '#A32D2D' : '#0F6E56', cursor: 'pointer' }}>
