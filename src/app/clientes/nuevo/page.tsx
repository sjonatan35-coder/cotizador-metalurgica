...
export default function NuevoClientePage() {
  ...
  const esReferido = form.fuente === 'Referido / boca en boca'
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (!profile?.tenant_id) { toast.error('No se encontró el tenant'); setLoading(false); return }
      ...
      if (esReferido && form.referido_por.trim()) {
        payload.notas = `Referido por: ${form.referido_por.trim()}${form.notas ? '\n' + form.notas : ''}`
      }
      ...
      setTimeout(() => router.push(`/clientes/${data.id}`), 800)
    } catch (err) {
      console.error(err)
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-brand-navy text-white p-4 flex items-center gap-3 sticky top-0 z-10">
        ...
        <div className="flex-1">
          <h1 className="text-sm font-bold leading-tight">Nuevo cliente</h1>
      ...
      <div className="p-4 flex flex-col gap-5 max-w-lg mx-auto pb-32">
        ...
        <section className="bg-slate-50 rounded-xl p-4 border-2 border-slate-200 flex flex-col gap-3">
          ...
          <div>
            ...
            <input type="text" placeholder="Ej: Juan Rodríguez" value={form.nombre}
          ...
          <div>
            ...
            <input type="text" placeholder="Ej: Herrería El Fortín" value={form.empresa}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="tel" placeholder="11 5555-0000" value={form.telefono}
                onChange={e => set('telefono', e.target.value)}
                className="w-full border-2 border-slate-300 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-blue-400" />
            </div>
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Email</label>
              <input type="email" placeholder="juan@mail.com" value={form.email}
          ...
          <div>
            ...
            <input type="text" placeholder="Ej: Villa Lugano, Matanza..." value={form.zona}
        ...
        <section className="bg-slate-50 rounded-xl p-4 border-2 border-slate-200 flex flex-col gap-3">
          ...
          {/* Motivo de pérdida — aparece solo si estado = perdido */}
          {esPerdido && (
            <div>
        ...
        <section className="bg-slate-50 rounded-xl p-4 border-2 border-slate-200 flex flex-col gap-3">
          ...
          {esCanalPago && (
            <div>
              <label className="text-xs text-slate-500 mb-1 block">¿Qué anuncio vio? (opcional)</label>
              <input type="text" placeholder="Ej: Video portón negro enero 2026" value={form.campana_origen}
                onChange={e => set('campana_origen', e.target.value)}
                className="w-full border-2 border-slate-300 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-blue-400" />
            </div>
          )}
          ...
          {esReferido && (
            <div>
              <label className="text-xs text-slate-500 mb-1 block">¿Quién lo recomendó?</label>
              <input type="text" placeholder="Nombre del cliente que refirió" value={form.referido_por}
        ...
        <section className="bg-slate-50 rounded-xl p-4 border-2 border-slate-200 flex flex-col gap-3">
          ...
          <div>
            ...
            <input type="text" placeholder="Ej: Llamar para confirmar medidas" value={form.proxima_accion_texto}
              onChange={e => set('proxima_accion_texto', e.target.value)}
              className="w-full border-2 border-slate-300 rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-blue-400" />
          </div>
          <div>
        ...
        <section className="bg-slate-50 rounded-xl p-4 border-2 border-slate-200 flex flex-col gap-3">
          ...
          <textarea placeholder="Ej: Busca portón corredizo, tiene medidas. Llama en la tarde." value={form.notas}
            onChange={e => set('notas', e.target.value)} rows={3}
